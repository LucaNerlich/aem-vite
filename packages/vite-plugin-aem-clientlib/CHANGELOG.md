...
### Changed
- Build migrated from `tsc` to `vp pack` ([Vite+](https://viteplus.dev)'s tsdown/Rolldown-based library bundler); `main`/`types`/`exports` now point at `dist/index.mjs`/`dist/index.d.mts` instead of the `.js`/`.d.ts` equivalents.
...
- **Golden descriptor tests verified unaffected**: `test/golden.test.ts` imports from `src/`, not `dist/`, so the build-tool swap cannot change what it asserts — re-run and confirmed byte-for-byte identical against the fixtures in `test/__golden__/` after migration.
