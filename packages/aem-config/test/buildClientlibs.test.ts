...
`;
  ...
  it('emits .content.xml with dependencies for the site clientlib', async () => {
    const xml = await readFile(path.join(outRoot, 'clientlib-site', '.content.xml'), 'utf8');
...
describe('buildClientlibs (sourcemap)', () => {
  ...
  beforeAll(async () => {
    ...
    await mkdir(path.join(dir, 'src'), { recursive: true });
    await writeFile(path.join(dir, 'src', 'main.ts'), 'export const value: number = 1;\n');
...
`;
  ...
  it('emits site.js.map under resources/sourcemaps/ (not next to site.js)', () => {
    expect(existsSync(path.join(outRoot, 'clientlib-site', 'js', 'site.js'))).toBe(true);
    expect(
      existsSync(path.join(outRoot, 'clientlib-site', 'resources', 'sourcemaps', 'site.js.map')),
    ).toBe(true);
    ...
    expect(existsSync(path.join(outRoot, 'clientlib-site', 'js', 'site.js.map'))).toBe(false);
  });
  ...
  it('rewrites sourceMappingURL to the resources/sourcemaps path AEM serves', async () => {
    const js = await readFile(path.join(outRoot, 'clientlib-site', 'js', 'site.js'), 'utf8');
    expect(js).toContain('sourceMappingURL=clientlib-site/resources/sourcemaps/site.js.map');
...
`,
  ...
  it('refuses to wipe an outDir that contains the config file', async () => {
    ...
    await expect(buildClientlibs({ mode: 'production', configPath, outDir: '.' })).rejects.toThrow(
      /outDir/,
    );
