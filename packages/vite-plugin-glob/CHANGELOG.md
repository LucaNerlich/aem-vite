...
### Removed
- **Breaking:** `ExpandOptions.cwd`/`sort` are gone — neither was ever set by the plugin itself or exercised by any test. `expandStyleGlobs`/`expandStyleGlobsWithResult` no longer take an options argument; glob resolution always uses `dirname(fromFile)` and results are always sorted lexicographically (identical to the previous default behavior).

## [0.10.0] - 2026-09-15

### Removed
- **Breaking:** dropped the redundant `export default aemViteGlob` alongside the named export. Only the named export (`import { aemViteGlob } from '@aemvite/vite-plugin-glob'`) was ever documented or used internally.
