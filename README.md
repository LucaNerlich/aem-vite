...
```jsonc
{
  "type": "module",
  "devDependencies": {
    "@aemvite/aem-config": "^0.9.0", // pulls the five plugin packages + esbuild peer
    "sass": "^1.77.0",
    "vite": "^8.1.0",
    "vitest": "^4.1.9",
...
- **`@aemvite/aem-config`**: `0.9.0` — self-sufficient orchestrator, `plugins`/`vite` passthrough, all five plugin packages now transitive deps.
- **`@aemvite/vite-plugin-aem-clientlib`**, **`@aemvite/vite-plugin-glob`**, **`@aemvite/vite-plugin-aem-resources`**, **`@aemvite/vite-plugin-aem-css-url-passthrough`**, **`@aemvite/vite-plugin-aem-handlebars`**: `0.9.0`.
