...
- `packages/` — the four published packages (workspaces). Each builds with `tsc` to `dist/`, tests with Vitest.
  - `aem-config` — the entry point. Typed config helper (`defineAemConfig`), config loader, build-options resolver, and the `aem-build` CLI orchestrator (`buildClientlibs`). Depends on all three plugin packages (`vite-plugin-aem-clientlib`, `vite-plugin-glob`, `vite-plugin-aem-resources`) and wires them into each clientlib's Vite build, so consumers install only `@aemvite/aem-config`.
