...
Tooling is plain npm + standalone packages, not a bundled meta-toolchain: each package builds with [`tsdown`](https://tsdown.dev) (Rolldown) to `dist/`, tests with [Vitest](https://vitest.dev), lints with [Oxlint](https://oxc.rs/docs/guide/usage/linter) (type-aware, via a root `.oxlintrc.json`), and formats with [Oxfmt](https://oxc.rs/docs/guide/usage/formatter) (via a root `.oxfmtrc.json`). Every package keeps a real `vite` dependency for type imports (`Plugin`/`UserConfig`/etc.) and, in `aem-config`, for Vite's programmatic build API. See "The byte-identical contract" below.
...
```sh
npm run build         # tsdown all packages, in dependency order (build --workspaces)
npm test              # vitest run, all packages (test --workspaces --if-present)
npm run lint          # oxlint, all packages (lint --workspaces --if-present)
npm run format:check  # oxfmt --check, whole repo
```
...
- **npm 10.9.2 Arborist crash on a from-scratch install.** `oxlint`/`oxfmt` declare `vite-plus` as an _optional_ peer dependency (harmless — nothing here uses it; a plain `npm install` won't pull it in). But a fully clean install (`rm -rf node_modules package-lock.json && npm install`) on npm 10.9.2 can crash with `TypeError: Cannot read properties of null (reading 'edgesOut')` inside `@npmcli/arborist`'s peer-set resolution, triggered by vitest's own optional-peer chain (`@vitest/browser*`). Workaround: install with a newer npm, e.g. `npx --yes npm@11 install` (matches what CI does via `npm install -g npm@latest`), then continue using `command npm` for everyday scripts.
- **Publishing is automated via CI**, not manual. Pushing a `v*` tag triggers `.github/workflows/publish.yml`, which uses npm OIDC trusted publishing (no NPM_TOKEN/OTP) and publishes in mandatory dependency order: `vite-plugin-aem-clientlib` → `vite-plugin-glob` → `vite-plugin-aem-resources` → `vite-plugin-aem-css-url-passthrough` → `vite-plugin-aem-handlebars` → `aem-config` (last, since it depends on the other five being resolvable). The README's manual `npm publish` section is a fallback. `prepublishOnly` runs `tsdown` automatically.
...
- **Node engines** are pinned to `^20.19.0 || ^22.18.0 || >=24.11.0` (required by `tsdown`; narrower than plain Vite 8's range — the 22.12–22.17 band is _not_ supported). `vite` and `esbuild` are peer/optional-peer deps — consumers must install them explicitly. A `.nvmrc` (`24`) is provided.
...
- `tsdown` (Rolldown) emits `.mjs`/`.d.mts` (not `.js`/`.d.ts` like the previous `tsc` build) — `main`/`types`/`exports`/`bin` fields in each package.json point at the `.mjs`/`.d.mts` paths accordingly. Peer/dev deps declared in package.json (`vite`, `handlebars`) are automatically externalized, never bundled.
- Linting is [Oxlint](https://oxc.rs/docs/guide/usage/linter), configured by a single root `.oxlintrc.json` (`options: { typeAware: true, typeCheck: true }`, requiring the `oxlint-tsgolint` devDependency). Each package's `lint` script invokes it explicitly against that shared config — `oxlint -c ../../.oxlintrc.json .` — rather than relying on upward config discovery. Ruleset is otherwise Oxlint's defaults, kept intentionally conservative — raising warnings to errors and fixing any backlog is a deliberate future follow-up, not part of this migration.
- Formatting is [Oxfmt](https://oxc.rs/docs/guide/usage/formatter), configured by a root `.oxfmtrc.json` (`singleQuote: true` to match the codebase's dominant style). `test/__golden__` and `test/fixtures` directories are excluded via `ignorePatterns` — they're byte-sensitive fixtures, not source to reformat. Run `npm run format` / `npm run format:check` from the root.
- **Lint must run after build, not before.** `typeAware`/`typeCheck` are on, so `aem-config`'s lint resolves the other five packages' types from their built `dist/*.d.mts` — running `oxlint` against a clean checkout before `npm run build` fails with `TS2307: Cannot find module '@aemvite/...'`. `.github/workflows/publish.yml` builds first, then lints, then tests.
